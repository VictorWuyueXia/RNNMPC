...
void setProblemType(d_struct_T *obj, int32_T PROBLEM_TYPE)
{
  int32_T i;
  int32_T idx_col;
  int32_T idx_row;
  ...
  case 2: {
    ...
    if (obj->probType != 4) {
      ...
      for (idx_col = 0; idx_col < mEq; idx_col++) {
        ...
        for (idx_row = offsetIneq_tmp; idx_row <= offsetEq1_tmp; idx_row++) {
        ...
        mIneq = i1 - 1;
        for (idx_row = i; idx_row <= mIneq; idx_row++) {
          obj->Aeq->data[idx_row + colOffsetAineq] = 0.0;
          obj->ATwset->data[idx_row + colOffsetATw] = 0.0;
        }
        obj->Aeq->data[i1 + colOffsetAineq] = -1.0;

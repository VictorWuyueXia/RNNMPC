...
real_T computeFval_ReuseHx(const g_struct_T *obj, emxArray_real_T *workspace,
                           const emxArray_real_T *f, const emxArray_real_T *x)
{
  ...
  case 3: {
    if (obj->hasLinear) {
      ...
      for (b_i = 0; b_i <= vectorUB; b_i += 2) {
        ...
        r1 = _mm_loadu_pd(&f_data[b_i]);
        _mm_storeu_pd(&workspace_data[b_i],
                      _mm_add_pd(_mm_mul_pd(_mm_set1_pd(0.5), r), r1));
      }
      ...
      if (obj->nvar < 1) {
        ...
        incx_t = (ptrdiff_t)1;
        incy_t = (ptrdiff_t)1;
        d = ddot(&n_t, (real_T *)&x_data[0], &incx_t, &obj->Hx->data[0],
                 &incy_t);
      }
      val = 0.5 * d;
    }
  ...
  case 4: {
    ...
    if (obj->hasLinear) {
      ...
      for (b_i = scalarLB; b_i < maxRegVar_tmp; b_i++) {
      ...
      val = 0.5 * d;
      i = obj->nvar + 1;
      for (scalarLB = i; scalarLB <= maxRegVar_tmp; scalarLB++) {
        val += x_data[scalarLB - 1] * obj->rho;
      }
    }
  } break;
  }
